...
public class PoseDetection : MonoBehaviour
{
    ...
    private bool isDetecting = false; // Prevent overlapping detections
    ...
    public async void Start()
    {
        ...
        while (true)
        {
            try
            {
                if (webCamTextureManager.WebCamTexture != null && !isDetecting)
                {
                    isDetecting = true;
                ...
                {
                    await Task.Delay(10); // Replace Awaitable.Delay with Task.Delay
                }
            }
    ...
    // Public method to set a texture for detection (can be called from CameraViewerManager)
    public async void SetTexture(Texture2D texture)
    {
        if (texture != null && !isDetecting)
        {
            isDetecting = true;
            m_DetectAwaitable = Detect(texture);
            await m_DetectAwaitable;
            isDetecting = false;
        }
    }
    ...
    void OnDestroy()
    {
        ...
        if (m_DetectorInput != null)
        {
            m_DetectorInput.Dispose();
        }
        if (m_LandmarkerInput != null)
        {
            m_LandmarkerInput.Dispose();
        }
    }
